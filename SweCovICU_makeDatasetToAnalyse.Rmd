...
swecovdata <- comorbiddata %>% 
  ...
  mutate(Age65plus = Age >= 65,
         ...
         DaysToDeath = as.Date(OffRegistryDate) - as.Date(AdmissionTime) - (`DaysFromDischargeToOff-registry/Diseased` %>% parse_number()) 
         ) %>% 
  select(-c(PersonalID,
            Deceased,
            ArrivalBMI,
...
# swecovdata %>%
#   group_by(DischargedTo,CareResult,Mortality30days) %>%
#   tally() %>% kable() %>% kable_styling(full_width = F)
