...
# xtabs(  ~ Mortality30days + `Invasive mechanical ventilation n (%)`, table2data)
...
table2 <- table2results %>% 
  mutate(
    ...
    Survivors = if_else(SurvivorsN > 10 & `Non-survivorsN` > 10, 
                  paste0(SurvivorsN, ' (', round(100*SurvivorsN/AllSurvivors,1), '%)'),
                  '-'),
    `Non-survivors` = if_else(SurvivorsN > 10 & `Non-survivorsN` > 10, 
                  paste0(`Non-survivorsN`, ' (', round(100*`Non-survivorsN`/AllNonSurvivors,1), '%)'),
                  '-'),
...
table3steps <- tibble(
  ...
  UnitStep = case_when(Variable == 'Age' ~ '20 years',
                       ...
                       Variable == 'PAO2/FIO2 on admission' ~ '10 UNITS???',
                       TRUE ~ '')
                      )
...
multimodel <- glm(Mortality30days ~ `Chronic heart diseases` +
                   `Chronic renal disease` + 
                   `Immunosuppression` + 
                   `Time from symptom onset to ICU` + 
                   `Acute respiratory failure` + 
