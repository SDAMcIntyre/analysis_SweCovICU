...
table3data <- swecovdata %>% 
  select(Mortality30days, 
         ...
         `Chronic renal disease` = RiskChronKidneyDisease, 
         `Immunosuppression` = RiskReducedImmuneDefence, 
         `Days from symptom onset to ICU` = DaysToICUIllnessCategory, 
         `PAO2/FIO2 on admission` = OxygenationIndex, 
         `Acute Kidney Injury` = KDIGOMin,
         `Acute respiratory failure` = J809worst, 
...
sensitivityadmission <- swecovdata %>% 
  select(Mortality30days, 
         ...
         CRRT, 
         `Transfer to other ICU` = ICUtransferred) %>% 
  mutate(`SAPS III score` = `SAPS III score`/5)
...
sensitivityBMI <- swecovdata %>% 
  select(Mortality30days, 
         `Obesity (degree)` = BMIcategory, # added this
         `Age Group` = AgeGroup,
         Sex = Gender,
         `SAPS III score` = SAPS3TotalPoints,
         `Diabetes Mellitus` = RiskDiabetes,
...
multimodel.int <- glm(Mortality30days ~
                    `Obesity (degree)` +
                    `Age Group` * `Obesity (degree)` +
                     Sex * `Obesity (degree)` +
                     `SAPS III score` * `Obesity (degree)` +
                     `Diabetes Mellitus` * `Obesity (degree)` +
                     Hypertension * `Obesity (degree)` +
                     `Chronic heart diseases` * `Obesity (degree)` + 
                     `Chronic lung disease` * `Obesity (degree)` + 
                     `Chronic renal disease` * `Obesity (degree)` + 
                     `Immunosuppression` * `Obesity (degree)` + 
                     `Days from symptom onset to ICU` * `Obesity (degree)` + 
                     `Acute respiratory failure` * `Obesity (degree)` + 
                     `Invasive mechanical ventilation` * `Obesity (degree)` + 
                     `Noninvasive mechanical ventilation` * `Obesity (degree)` + 
                     HFNO * `Obesity (degree)` + 
                     `Prone positioning` * `Obesity (degree)` + 
                     `Specific pharmacotherapy*` * `Obesity (degree)` + 
                     CRRT * `Obesity (degree)` + 
                     `Transfer to other ICU` * `Obesity (degree)`,
              family=binomial(link="logit"),
              data = sensitivityadmission)
